...
from dewey.packaged_docs import agent as packaged_agent_doc
from dewey.packaged_docs import meta as packaged_meta_doc
from dewey.packaged_docs import readme as packaged_readme_doc
...
def current_repo_or_none() -> DeweyRepo | None:
    try:
        return DeweyRepo.discover()
    except DeweyError:
        return None
...
def repo_counts(repo: DeweyRepo) -> dict[str, int]:
    ...
    return {
        ...
        "ordered_sources": len(repo.load_order().order),
        "total_links": count_total_links(repo),
        "stale_or_failed_markdown": stale_or_failed,
    }


def agent_start_state(repo: DeweyRepo | None) -> tuple[str, list[str], dict[str, int]]:
    if repo is None:
        return (
            "no-project",
            ["dewey init"],
    ...
    if counts["sources"] == 0:
        return ("setup", ["dewey add source paper.bib", "dewey add source paper.pdf"], counts)
    if counts["with_markdown_ready"] == 0 and counts["with_pdf"] > 0:
        return ("render", ["dewey render md --all", "dewey status"], counts)
    if counts["ordered_sources"] == 0:
        return ("organize", ["dewey order set <source-id> ...", "dewey state set <source-id> included"], counts)
    return ("active", ["dewey status", "dewey search <query>", "dewey show <source-id>"], counts)


@app.command("meta")
def meta_command() -> None:
    typer.echo(packaged_meta_doc(), nl=False)


@app.command("agent")
def agent_command() -> None:
    typer.echo(packaged_agent_doc(), nl=False)


@app.command("readme")
def readme_command() -> None:
    typer.echo(packaged_readme_doc(), nl=False)


@app.command("agent-start")
def agent_start(json_output: bool = typer.Option(False, "--json")) -> None:
    action = "agent.start"
    repo = current_repo_or_none()
    state, next_commands, counts = agent_start_state(repo)
    project_dir = str(repo.root) if repo is not None else str(Path.cwd())
    payload = {
        "ok": True,
        "action": action,
        "state": state,
        ...
        "counts": counts,
        "agent_brief": packaged_meta_doc().strip(),
        "agent": packaged_agent_doc(),
        "readme": packaged_readme_doc(),
        "primary_doc": "AGENT.md",
        "useful_commands": next_commands,
        "text": f"Dewey state: {state}",
    }
