...
function filters() {
  ...
  return `<div class="toolbar"><div class="search-row"><input id="q" type="search" aria-label="Search ${state.tab==='candidates'?'screening records':'literature'}" placeholder="Search titles, authors, findings…" value="${esc(state.q)}"><button data-action="reset">Reset filters</button></div><details class="filter-box"><summary>Filters${['sources','evidence'].includes(state.tab)?' · '+esc(state.status||'all sources'):''}${state.theme||state.design||state.provider||state.reason?' · active':''}</summary><div class="filter-grid">${fields}</div>${state.tab==='graph'&&state.q?'<p class="meta filter-note">Search shows matching papers and their neighbors across the corpus. Clear search to use the focus controls.</p>':''}</details></div>`;
}

function overview() {
  const included=D.sources.filter(s=>s.status==='included');
  const countLink=(n,label,patch)=>link(n+' '+label,{...clearDetail,...patch});
  const counts=[countLink(included.length,'included papers',{tab:'sources'}),countLink(D.studies.length,'studies',{tab:'evidence'}),countLink(D.findings.length,'findings',{tab:'evidence'}),countLink(D.sources.length,'total sources',{tab:'sources',status:''})].join(' · ');
  const conclusions=D.claims.map(c=>`<tr class="claim-card"><td><h3>${claimLink(c.claim_id)}</h3><p class="scope">${esc(c.scope)}</p></td><td><span class="confidence">${esc(c.confidence)}</span><span class="meta"> · ${esc(c.status)}</span><p class="evidence-counts">${c.evidence.filter(e=>e.relationship==='supports').length} supporting / ${c.evidence.filter(e=>e.relationship==='qualifies').length} qualifying / ${c.evidence.filter(e=>e.relationship==='contradicts').length} contradictory</p><p class="meta">${[...new Set(c.evidence.map(e=>findings.get(e.finding_id)?.study_id).filter(Boolean))].map(id=>studyLink(id,studySources(studies.get(id)).map(p=>p.citation_label).join('; '))).join('; ')}</p></td></tr>`).join('');
  const coverage=[['summarized',included.filter(s=>s.summary).length],['with stored full text',included.filter(s=>s.markdown||s.has_pdf).length],['with study extraction',included.filter(s=>sourceStudies(s.source_id).length).length],['with full-text reading history',included.filter(s=>s.last_read_at&&s.read_depth==='full-text').length]].map(([label,n])=>`${n}/${included.length} ${label}`).join(' · ');
  return `<div class="section-head"><h2 tabindex="-1">Conclusions</h2><p class="record-counts">${counts}</p></div>
    ${conclusions?`<div class="table-wrap"><table class="conclusion-table"><thead><tr><th>Conclusion and scope</th><th>Confidence, evidence, and studies</th></tr></thead><tbody>${conclusions}</tbody></table></div>`:empty('No synthesis claims recorded. Explore the papers and study evidence.')}
    <section class="coverage"><h3>Coverage</h3><p>${coverage}.</p><p class="meta">${D.claims.filter(c=>c.status==='draft').length} draft claims. Missing reading history does not establish that a paper was never read. ${link('Screening and search coverage →',{...clearDetail,tab:'candidates'})}</p></section>
    <div class="theme-index"><strong>Themes:</strong> ${D.themes.map(t=>link(t.label,{...clearDetail,tab:'evidence',theme:t.theme_id,page:'1'})).join(' · ')||'None recorded'}</div>
    <details class="detail-section"><summary>Scope, theme definitions, and interpretation</summary><p>Findings separate authors’ claims, evidence, and reviewer interpretation. Measures differ between studies; estimates are not pooled.</p><dl class="facts">${D.themes.map(t=>`<dt>${esc(t.label)}</dt><dd>${esc(t.description)}</dd>`).join('')}</dl><div class="prose">${markdown(D.instructions)}</div></details>`;
}
...
function evidenceTable() {
  ...
  return `<div class="section-head"><div><h2 tabindex="-1">Studies</h2><p class="muted">${rows.length} / ${D.studies.length} studies · A paper may contain multiple studies.</p></div><div class="actions"><button data-action="csv">Download evidence CSV</button><button data-action="json">Export this view (JSON)</button></div></div>
...
function paperList() {
  ...
  return `<div class="section-head"><h2 tabindex="-1">Papers <span class="meta">${rows.length} / ${D.sources.length}</span></h2><button data-action="bibtex">Download BibTeX</button></div><div class="paper-list">${pageRows(rows).map(s=>`<article class="card paper-row"><div class="citation">${esc(s.citation_label)}</div><div><h3>${sourceLink(s.source_id,s.title)}</h3><p class="teaser">${esc(s.summary || 'No summary recorded.')}</p>${s.metadata_incomplete?'<span class="meta">Incomplete metadata</span>':''}</div><div class="paper-status">${badge(s.status)}<span>${s.markdown?'Extracted text':s.has_pdf?'PDF':'No full text'}</span><span>${sourceStudies(s.source_id).length} ${sourceStudies(s.source_id).length===1?'study':'studies'}</span>${safePDF(s)?`<a href="${esc(safePDF(s))}" target="_blank" rel="noopener">PDF ↗</a>`:''}</div></article>`).join('')||empty('No papers match these filters.')}</div>${pagination(rows.length)}`;
}
...
function screeningAudit() {
  ...
  const summaries=(title,values)=>`<section><h4>${title}</h4><ul class="count-list">${countBy(values).map(([label,n])=>`<li><span>${esc(words(label))}</span><strong>${n}</strong></li>`).join('')||'<li>Not recorded</li>'}</ul></section>`;
  return `<div class="section-head"><h2 tabindex="-1">Screening audit</h2><button data-action="json">Export this view (JSON)</button></div><p class="record-counts">${counts.map(([label,n])=>link(n+' '+words(label),{...clearDetail,candidateStatus:label,page:'1'})).join(' · ')} · ${rows.length} matching</p>
  <details class="detail-section audit-methods"><summary>Search coverage, provenance, and decision totals</summary><p>Candidates are possible sources found through searches and citation traversal. A resolved queue does not establish complete search coverage; “added” counts candidate records, not unique corpus papers.</p><div class="grid">${summaries('Candidate origins (may overlap)',D.candidates.flatMap(c=>[...new Set(c.provenance.map(p=>p.provider||p.method))]))}${summaries('Latest recorded reason',D.candidates.map(c=>c.screening_decisions.at(-1)?.reason_code))}</div>${searchCoverage()}</details>
  <div class="table-wrap"><table class="screening-table"><thead><tr><th>Candidate</th><th>Decision</th><th>Rationale</th></tr></thead><tbody>${pageRows(rows).map(c=>`<tr class="card"><td><h3>${link(c.title||'Unresolved citation',{...clearDetail,candidate:c.candidate_id})}</h3><p class="meta">${esc(Array.isArray(c.authors)?c.authors.join('; '):c.authors)}${c.year?' · '+esc(c.year):''}</p></td><td>${badge(c.status)}</td><td>${esc(c.screening_decisions.at(-1)?.rationale||c.rationale||'No rationale recorded.')}</td></tr>`).join('')||'<tr><td colspan="3">No matching candidates.</td></tr>'}</tbody></table></div>${pagination(rows.length)}`;
}
...
function graphView() {
  ...
  const width=1050, step=44, height=Math.max(140,nodes.length*step+35), positions=new Map(nodes.map((s,i)=>[s.source_id,{x:40+(i%3)*330,y:28+i*step}]));
