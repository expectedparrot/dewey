...
<label id="graph-scope" class="graph-scope hidden"><input id="show-all-citations" type="checkbox"> Show unread and unresolved citation leads</label>
...
function citationGraph(edges){{
  const ids=showAllCitations.checked?[...new Set(edges.flatMap(x=>[x.from,x.to]))]:D.sources.filter(x=>x.status==='included').map(x=>x.source_id), source=id=>D.sources.find(x=>x.source_id===id), year=id=>parseInt(source(id)?.year)||9999;
