...
def citation_label(authors: str, year: str) -> str:
...
def explorer_payload(repo: DeweyRepo) -> dict[str, Any]:
        ...
        sources.append(
            {
                ...
                "year": entry.year(),
                "citation_label": citation_label(entry.author(), entry.year()),
    ...
    for group in label_groups.values():
        ...
        for index, source in enumerate(sorted(group, key=lambda item: item["title"].casefold())):
            source["citation_label"] = source["citation_label"].removesuffix(")") + f"{chr(97 + index)})"
    return {
...
const q=document.getElementById('search'), filter=document.getElementById('filter'), list=document.getElementById('list'), detail=document.getElementById('detail'), note=document.getElementById('tab-note'), bibButton=document.getElementById('bibtex');
...
function citationGraph(edges){{
  const ids=[...new Set(edges.flatMap(x=>[x.from,x.to]))], source=id=>D.sources.find(x=>x.source_id===id), year=id=>parseInt(source(id)?.year)||9999;
